...
foreach my $ctype ( 'text/plain', 'application/json' ){
  ...
  is  get(encode_utf8("foo\x{FFFF_FFFF}bar")), encode_utf8("foo\x{FFFF_FFFF}bar"), "encoded lax utf8 character comes back as is";
  like pop(@warnings), qr/does not map to Unicode/, 'encode croaked';
  is  get("foo\x{FFFF_FFFF}bar"), "foo\x{FFFF_FFFF}bar", "unencoded lax utf8 character comes back as is";
  ...
  # HEAVY BLACK HEART
  is get($filedata), "i \x{2764} metacpan", 'correct message';
  ok !@warnings, 'no warnings after valid UTF-8' or diag shift @warnings;
}
